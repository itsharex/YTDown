...
pub async fn get_ytdlp_info(state: State<'_, AppState>) -> Result<YtdlpInfo, String> {
    ...
    let binary = binary::detect_binary(manual_path)?;
    let update_available = matches!(binary.managed_by, binary::ManagedBy::Homebrew)
        && binary::check_homebrew_update().unwrap_or(false);

    let managed_by = match binary.managed_by {
